...
beforeEach(^{
  ...
  // actor with routing
  {
      ITLinkNode *const rootNode = OCMPartialMock(
          [ITLinkNode linkActionWithModuleName:@"RootModule" link:@selector(navigateToA) arguments:nil router:router]);
      ...
      ITLinkNode *const drootNode =
          OCMPartialMock([ITLinkNode linkActionWithNode:rootNode link:@selector(navigateToB:) arguments:@[ @"Bob" ]]);
  ...
  {
      ITLinkNode *const rootNode =
          OCMPartialMock([ITLinkNode linkActionWithModuleName:@"RootModule" link:@selector(navigateToA) arguments:nil]);
      ...
      actorWithoutRouter =
          [[ITNavigationActor alloc] initWithSourceChain:sourceChain destinationChain:destinationChain];
...
describe(@"navigation cycle",
         ^{
           it(@"should work with valid data", ^{
             ...
             [actor runNextWithCurrentDirection:ITLinkNavigationTypeBack
                                        andNode:[ITLinkNode linkValueWithModuleName:@"RootModule"]];
             expect(actor.isStarted).to.beTruthy();
             expect(actor.isFinished).to.beFalsy();
             OCMExpect([delegateMock didFinishNavigation:actor]);
             [actor runNextWithCurrentDirection:ITLinkNavigationTypeForward
                                        andNode:[ITLinkNode linkValueWithModuleName:@"LoginWithCModule"]];
...
describe(@"misusing navigation methods",
         ^{
           ...
           it(@"when trying to start navigation by not using `start` method should result nothing", ^{
             ...
             [OCMReject(delegateMock) didFinishNavigation:actor];
             [actor runNextWithCurrentDirection:ITLinkNavigationTypeBack
                                        andNode:[ITLinkNode linkValueWithModuleName:@"LoginModule"]];
             expect(actor.isStarted).to.beFalsy();
             expect(actor.isFinished).to.beFalsy();
           });
           ...
           it(@"when passing invalid navigation type during navigation should produce exception", ^{
             ...
             [actor runNextWithCurrentDirection:ITLinkNavigationTypeBack
                                        andNode:[ITLinkNode linkValueWithModuleName:@"RootModule"]];
             expect(^{
               [actor runNextWithCurrentDirection:(ITLinkNavigationType)4
                                          andNode:[ITLinkNode linkValueWithModuleName:@"LoginWithCModule"]];
